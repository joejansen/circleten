...
function scene:enterScene( event )
	...
	local oneFifthY = (maxVisibleY-roofHeight)/5
	local xWidth = floorRoofWidth-65
	
	local levelNum = 24
	local movesNum = 15
	local goalNum = 2
	local cueBallStartX = display.screenOriginX+125
	local cueBallStartY = display.screenOriginY+4*oneFifthY+50
...
----------------OPTIONS
	...
	local orangeWall1 = gameLogic.createOrangeWall(rightXBound-xWidth,roofHeight+oneFifthY,xWidth,1)
	wallGroup:insert(orangeWall1)

	local orangeWall2 = gameLogic.createOrangeWall(display.screenOriginX,roofHeight+2*oneFifthY,xWidth,1)
	wallGroup:insert(orangeWall2)
	
	local orangeWall3 = gameLogic.createOrangeWall(rightXBound-xWidth,roofHeight+3*oneFifthY,xWidth,1)
	wallGroup:insert(orangeWall3)	

	local orangeWall4 = gameLogic.createOrangeWall(display.screenOriginX,roofHeight+4*oneFifthY,xWidth,1)
	...
	-- 
	local specificStageBall1 = gameLogic.createSpecificStageBall(8,display.screenOriginX+70,cueball.y)
	allball:insert(specificStageBall1)
	
	local specificStageBall2 = gameLogic.createSpecificStageBall(7,display.screenOriginX+35,cueball.y)
	allball:insert(specificStageBall2)
	-- 
	local specificStageBall3 = gameLogic.createSpecificStageBall(2,rightXBound - 70,orangeWall1.y-30)
	allball:insert(specificStageBall3)
	
	local specificStageBall4 = gameLogic.createSpecificStageBall(3,rightXBound - 35,orangeWall1.y-30)
